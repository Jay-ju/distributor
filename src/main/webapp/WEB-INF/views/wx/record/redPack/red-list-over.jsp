...
<link rel="stylesheet" type="text/css"
	href="${ctx}/static/wx/css/compute.css" />
		...
		<div class="content">
			<ul id="dataList">
				<c:forEach items="${page.result}" var="entity">
					<li class="line">
						<div class="conLeft p-plus">${entity.total}元</div>
						<div class="conRight larger">
							<h5><c:choose><c:when test="${entity.category==1}">夺宝红包</c:when><c:otherwise>生活红包</c:otherwise></c:choose></h5>
                   			<p class="normal text-low2">
								余额：<b class="pink">${entity.balance}夺宝币</b>
							</p>
							<p class="normal text-low2">有效期：<fmt:formatDate value="${entity.expireTime}" pattern="yyyy-MM-dd HH:mm"/></p>
							<c:choose>
								<c:when test="${entity.state==3}">
									<ins>
										<img src="${ctx}/static/wx/images/pay/expire.png" alt="">
									</ins>
									<!-- 已过期 -->
								</c:when>
								<c:otherwise>
									<ins>
										<img src="${ctx}/static/wx/images/pay/apply .png" alt="">
									</ins>
									<!-- 已使用 -->
								</c:otherwise>
							</c:choose>
    ...
    wxcommon.autoLoad({
        ...
        doRowDataFun: function(rowData){                // 格式化行级数据
        	var str = '<li class="line">'+
                '<div class="conLeft p-plus">'+rowData.total+'元</div>'+
         '<div class="conRight larger"><h5>';
           ...
           if(rowData.state==3){
        	   str = str+'<ins><img src="${ctx}/static/wx/images/pay/expire.png" alt=""></ins>';
           }else{
        	   str = str+'<ins><img src="${ctx}/static/wx/images/pay/apply .png" alt=""></ins>';
           }
         str = str+'</div></li>';
        	return str;
        }
