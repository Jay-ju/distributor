...
import com.zx.stlife.tools.weixin.WeixinHttpUtils;
...
public class SnatchRecordService {
    ...
    public Map<String, Object> payOrOrders(SnatchRequestVo snatchRequestVo,
        ...
        boolean isHeePay = payWays.contains(PayWayType.HEEPAY);
        boolean isWeixinAppPay = payWays.contains(PayWayType.WEIXINAPP);
        boolean isIPSPay = payWays.contains(PayWayType.IPSPAY);
        ...
        } else if(isHeePay) {                // 汇付宝支付下单
            logger.info("==========第三方支付:汇付宝");
            return recordHeePayOrders(
            		totalCanUseRedPackList, thirdPartPayMoney, totalCanBuyTimes, user, request,
                    goodsTimesMap);
        } else if(isWeixinAppPay) {                // 微信APP支付下单
        	logger.info("==========第三方支付:微信APP");
            return recordWeixinAPPOrders(
                    totalCanUseRedPackList, thirdPartPayMoney, totalCanBuyTimes, user, request,
                    goodsTimesMap);
        } else if(isIPSPay) {                // 环迅支付下单
        	logger.info("==========第三方支付:环迅支付");
            return recordIPSAPPOrders(
                    totalCanUseRedPackList, thirdPartPayMoney, totalCanBuyTimes, user, request,
                    goodsTimesMap);
        } else {                // 支付宝支付下单
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public Map<String, Object> recordWeixinAPPOrders(
                                          List<RedPack> totalCanUseRedPackList, int thirdPartPayMoney,
                                          int totalCanBuyTimes, User user, HttpServletRequest request,
                                          Map<GoodsTimes, Integer> goodsTimesMap) throws Exception {

        String outTradeNo = SimpleUtils.getHibernateUUID();
        JsonResult jsonResult = WeixinUitls.orderByWeixinAppPay(
                request, user, outTradeNo, thirdPartPayMoney, true, false);

        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            // 预生成支付记录
            recordOrders(false, false, totalCanUseRedPackList,
                    thirdPartPayMoney, totalCanBuyTimes, user, goodsTimesMap, outTradeNo,
                    true, false, RequestUtils.getRealIp(request), false, true, false);
        }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public Map<String, Object> recordIPSAPPOrders(
                                          List<RedPack> totalCanUseRedPackList, int thirdPartPayMoney,
                                          int totalCanBuyTimes, User user, HttpServletRequest request,
                                          Map<GoodsTimes, Integer> goodsTimesMap) throws Exception {

        String outTradeNo = SimpleUtils.getHibernateUUID();
        JsonResult jsonResult = WeixinUitls.orderByIPSPay(
                request, user, outTradeNo, thirdPartPayMoney, true, false);

        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            // 预生成支付记录
            recordOrders(false, false, totalCanUseRedPackList,
                    thirdPartPayMoney, totalCanBuyTimes, user, goodsTimesMap, outTradeNo.substring(0, 32),
                    true, false, RequestUtils.getRealIp(request), false, false, true);
        }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public Map<String, Object> recordHeePayOrders(
                                          List<RedPack> totalCanUseRedPackList, int thirdPartPayMoney,
                                          int totalCanBuyTimes, User user, HttpServletRequest request,
                                          Map<GoodsTimes, Integer> goodsTimesMap) throws Exception {

        String outTradeNo = SimpleUtils.getHibernateUUID();
        JsonResult jsonResult = WeixinUitls.orderByHeeAppPay(
                request, user, outTradeNo, thirdPartPayMoney, true, false);
        ...
        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            ...
            recordOrders(false, false, totalCanUseRedPackList,
                    thirdPartPayMoney, totalCanBuyTimes, user, goodsTimesMap, outTradeNo,
                    true, false, RequestUtils.getRealIp(request), true, false, false);
        }
    ...
    public Map<String, Object> recordWeixinOrders(boolean isPayWithBalance, boolean isPayWithRedPack,
        ...
        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            ...
            recordOrders(isPayWithBalance, isPayWithRedPack, totalCanUseRedPackList,
                    thirdPartPayMoney, totalCanBuyTimes, user, goodsTimesMap, outTradeNo,
                    true, isH5Pay, RequestUtils.getRealIp(request), false, false, false);
        }
    ...
    public Map<String, Object> recordAlipayOrders(boolean isPayWithBalance, boolean isPayWithRedPack,
        ...
        try {
            recordOrders(isPayWithBalance, isPayWithRedPack, totalCanUseRedPackList,
                    thirdPartPayMoney, totalCanBuyTimes, user, goodsTimesMap, outTradeNo, false, false, ip, false, false, false);
    ...
    public void recordOrders(boolean isPayWithBalance, boolean isPayWithRedPack,
                              ...
                              String outTradeNo, boolean isWeixinPay, boolean isH5Pay, String ip, boolean isHeePay, boolean isWeixinAPPPay, boolean isIPSPAY) throws Exception {
        // 预生成支付记录
        PayRecord payRecord = null;
        if(isWeixinPay || isHeePay || isWeixinAPPPay || isIPSPAY){
        ...
        Byte way = PayWayType.ALIPAY;
        if (isWeixinPay) {
        	way = PayWayType.WEIXIN;
        } else if (isHeePay) {
        	way = PayWayType.HEEPAY;
        } else if (isWeixinAPPPay) {
        	way = PayWayType.WEIXINAPP;
        } else if (isIPSPAY) {
        	way = PayWayType.IPSPAY;
        }
        // 微信或支付宝支付
        PayWay payWay = new PayWay(way, thirdPartPayMoney, payRecord, null);
        payWayService.save(payWay);
        ...
        // 定时查询订单状态的任务
        if (isWeixinPay || isHeePay || isWeixinAPPPay || isIPSPAY){
            addQueryWeixinOrderJob(true, isH5Pay, payRecord.getOutTradeNo(), PayUtils.getQueryJobCronExpression(), isHeePay, isWeixinAPPPay, isIPSPAY);
        }
    }
 ...
 //           goodsTimes.setOpenTime(DateUtilsEx.addSeconds(date, 15)); // 多加4分钟再开奖
    ...
    public JsonResult rechargeOrders(User user, boolean isWeixinPay, boolean isWeixinPayWithH5, boolean isAlipayWithH5,
                                     Integer money, HttpServletRequest request,
                                     boolean isHeePay, boolean isWeixinAppPay, boolean isIPSPay) throws Exception {

        if (isWeixinPay) {          // 微信充值下单
            return rechargeOrdersByWeixin(user, money, request, isWeixinPayWithH5);
        } else if (isHeePay) {   // 汇付宝充值下单
            return rechargeOrdersByHeeAPPpay(user, money, request, isWeixinPayWithH5);
        } else if (isWeixinAppPay) {   // 微信APP充值下单
            return rechargeOrdersByWXAPPpay(user, money, request, isWeixinPayWithH5);
        } else if (isIPSPay) {   // 环迅支付充值下单
            return rechargeOrdersByIPSpay(user, money, request, isWeixinPayWithH5);
        } else{   // 支付宝充值下单
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public JsonResult rechargeOrdersByWXAPPpay(User user,
                Integer money, HttpServletRequest request, boolean isH5Pay) throws Exception {
        String outTradeNo = SimpleUtils.getHibernateUUID();
        JsonResult jsonResult = WeixinUitls.orderByWeixinAppPay(request, user, outTradeNo, money, false, false);
        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            ...
            //添加定时查询订单任务(TODO要修改)
            addQueryWeixinOrderJob(false, isH5Pay, rechargeRecord.getOutTradeNo(), PayUtils.getQueryJobCronExpression(), false, true, false);
        }
        return jsonResult;
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public JsonResult rechargeOrdersByIPSpay(User user,
                Integer money, HttpServletRequest request, boolean isH5Pay) throws Exception {
        String outTradeNo = SimpleUtils.getHibernateUUID();
        JsonResult jsonResult = WeixinUitls.orderByIPSPay(request, user, outTradeNo, money, false, false);
        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            RechargeRecord rechargeRecord = rechargeRecordService.save(money, true, user,
                    outTradeNo, true, PayUtils.getExpireDate(), RechargeState.UNRECHARGE);

            //添加定时查询订单任务(TODO要修改)
            addQueryWeixinOrderJob(false, isH5Pay, rechargeRecord.getOutTradeNo(), PayUtils.getQueryJobCronExpression(), false, false, true);
        }
        return jsonResult;
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public JsonResult rechargeOrdersByHeeAPPpay(User user,
                Integer money, HttpServletRequest request, boolean isH5Pay) throws Exception {
        String outTradeNo = SimpleUtils.getHibernateUUID();
        JsonResult jsonResult = WeixinUitls.orderByHeeAppPay(request, user, outTradeNo, money, false, false);
        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            RechargeRecord rechargeRecord = rechargeRecordService.save(money, true, user,
                    outTradeNo, true, PayUtils.getExpireDate(), RechargeState.UNRECHARGE);

            //添加定时查询订单任务(TODO要修改)
            addQueryWeixinOrderJob(false, isH5Pay, rechargeRecord.getOutTradeNo(), PayUtils.getQueryJobCronExpression(), true, false, false);
        }
        return jsonResult;
    }
    ...
    public JsonResult rechargeOrdersByWeixin(User user,
        ...
        if (jsonResult.getState() == AppStatusCode.SC_SUCCESS){
            ...
            //添加定时查询订单任务
            addQueryWeixinOrderJob(false, isH5Pay, rechargeRecord.getOutTradeNo(), PayUtils.getQueryJobCronExpression(), false, false, false);
        }
        return jsonResult;
    }
...
//            cqsscLuckNum = CQSSCUtils.getLuckNum(cqsscPeriodNo);
   ...
   //     long sleepMillis = goodsTimes.getOpenTime().getTime() - DateUtils.getNow().getTime() - 2000;
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public boolean queryHeePayResult(String outTradeNo, boolean isH5Pay) throws Exception{
        logger.info("查询汇付宝支付订单： PayRecord outTradeNo: {}", outTradeNo);
        PayRecord payRecord = payRecordService.getByOutTradeNo(outTradeNo);
        if(payRecord == null ||
                !payRecord.getNeedQueryPayResult() ||
                DateUtilsEx.getNow().compareTo(payRecord.getQueryDeadlineTime()) > 0 ){
        ...
        Object[] result = WeixinUitls.queryHeeAppPayResult(
                payRecord.getOutTradeNo(), HEEPAY_AGENT_KEY);
        if(result != null){
            payRecord.setNeedQueryPayResult(false);
            if( (boolean)result[0] ){
                computeBuyTimsAndSantch(payRecord.getOutTradeNo(), (Date)result[1]);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public boolean queryWxAppPayResult(String outTradeNo, boolean isH5Pay) throws Exception{
        logger.info("查询微信APP支付订单： PayRecord outTradeNo: {}", outTradeNo);
        PayRecord payRecord = payRecordService.getByOutTradeNo(outTradeNo);
        if(payRecord == null ||
                !payRecord.getNeedQueryPayResult() ||
                DateUtilsEx.getNow().compareTo(payRecord.getQueryDeadlineTime()) > 0 ){
        ...
        String requestXML = WeixinUitls.buildWxQueryOrderRequestXML(
                payRecord.getOutTradeNo(), WXAPPPAY_SECRET);
        logger.info("微信APP{}下单请求结果：{}", requestXML);
        String responseXML = WeixinHttpUtils.httpsRequest(WXAPPPAY_ORDER_URL, "POST", requestXML);
        logger.info("微信APP{}下单响应结果：{}", responseXML);
        ...
        if(resultMap.containsKey("sign")){
            if(isValidSign((TreeMap)resultMap, apiKey)){
            	...
            	if ("0".equals(resultMap.get("status"))
            		...
            		computeBuyTimsAndSantch(payRecord.getOutTradeNo(), payTime);
            		return true;
            	} else {
            		cancelPay(payRecord);
            		return false;
                ...
                logger.error("微信APP支付{}下单返回结果，签名错误。", "支付");
            }
        }else{
        	cancelPay(payRecord);
        	return false;
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public boolean queryIPSPayResult(String outTradeNo, boolean isH5Pay) throws Exception{
        logger.info("环迅支付订单： PayRecord outTradeNo: {}", outTradeNo);
        PayRecord payRecord = payRecordService.getByOutTradeNo(outTradeNo);
        if(payRecord == null ||
                !payRecord.getNeedQueryPayResult() ||
                DateUtilsEx.getNow().compareTo(payRecord.getQueryDeadlineTime()) > 0 ){
        ...
        String requestXML = WeixinUitls.buildWxQueryOrderRequestXML(
                payRecord.getOutTradeNo(), WXAPPPAY_SECRET);
        logger.info("环迅支付{}下单请求结果：{}", requestXML);
        String responseXML = WeixinHttpUtils.httpsRequest(WXAPPPAY_ORDER_URL, "POST", requestXML);
        logger.info("环迅支付{}下单响应结果：{}", responseXML);

        Map<String, String> resultMap = XMLUtils.doXMLParse(responseXML);
        if(resultMap == null){
            return false;
        }
        Date payTime = DateUtilsEx.toDateFromYYYYMMDDHHMMSS(null,
        		DateUtilsEx.getNow());
		computeBuyTimsAndSantch(payRecord.getOutTradeNo(), payTime);
		return true;
    }
     ...
     * @param cronExpression
     */
    private void addQueryWeixinOrderJob(boolean isPay, boolean isH5Pay,
    		String outTradeNo, String cronExpression,
    		boolean isHeePay, boolean isWxAppPay, boolean isIPSPAY){
        Map<String, String> params = new HashMap<String, String>(1); //0 2/10 * * * ?
        params.put("outTradeNo", outTradeNo);
        params.put("isH5Pay", String.valueOf(isH5Pay));
        params.put("isHeePay", String.valueOf(isHeePay));
        params.put("isWxAppPay", String.valueOf(isWxAppPay));
        params.put("isIPSPAY", String.valueOf(isIPSPAY));
     ...
     * @param cronExpression
     */
    private void addQueryHeeOrderJob(boolean isPay, boolean isH5Pay, String outTradeNo, String cronExpression){
        Map<String, String> params = new HashMap<String, String>(1); //0 2/10 * * * ?
        params.put("outTradeNo", outTradeNo);
        params.put("isH5Pay", String.valueOf(isH5Pay));
        QuartzEntity quartzEntity = null;
