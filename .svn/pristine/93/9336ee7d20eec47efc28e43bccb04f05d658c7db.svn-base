...
public class PayCallbackController extends BaseRestController {
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/wxapp/buy", method = RequestMethod.POST)
    public void wxAppPayCallBack(HttpServletRequest request, HttpServletResponse response)
            throws JDOMException, IOException {
        wxAppPayCallBack(request, response, false);
    }
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/ips/buy", method = RequestMethod.POST)
    public void ipsPayCallBack(HttpServletRequest request, HttpServletResponse response)
            throws JDOMException, IOException {
        ipsPayCallBack(request, response, false);
    }

    private void ipsPayCallBack(HttpServletRequest request, HttpServletResponse response, boolean isH5Pay)
            throws JDOMException, IOException {
        Map<String, String> params = RequestUtils.collectIPSXmlParamsToMap(request);
        logger.info("ipsAppPayCallBack========" + params.values());
        for (String key : params.keySet()) {
            logger.info("key= "+ key + " and value= " + params.get(key));
        }
        if (params == null || params.size() == 0) {
            logger.error("无法获取环迅支付成功回调结果");
            return;
        }

        if ("000000".equalsIgnoreCase(params.get("RspCode"))
                && "Y".equalsIgnoreCase(params.get("Status"))) {
            // 对订单的处理
            String outTradeNo = params.get("IpsBillNo");// 获取支付成功的商户订单号
            String timeEndStr = params.get("IpsBillTime");
            ...
            LogUtils.logMap("环迅支付回调 - 参数错误：", params);
        }
    }
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/wxapp/recharge", method = RequestMethod.POST)
    public void wxAppRechargeCallBack(HttpServletRequest request, HttpServletResponse response)
            throws JDOMException, IOException {
        //获取支付宝POST过来反馈信息
        Map<String, String> params = RequestUtils.collectXmlParamsToMap(request);
        logger.info("微信APP充值回调 - 处理成功，商户订单号：{}=============", params);
        if ("0".equalsIgnoreCase(params.get("status"))
                && "0".equalsIgnoreCase(params.get("result_code"))) {
                // 对订单的处理
                ...
                while (true) {
                    try {
                        ...
                        logger.info("微信APP充值回调 - 处理成功，商户订单号：{} - end", outTradeNo);
                        break;
                    } catch (ObjectOptimisticLockingFailureException | StaleObjectStateException ex) {
                        logger.info("微信APP充值回调 - 出现并发，商户订单号：{}", outTradeNo);
                        Threads.sleep(RandomUitls.randomInt(100));
                    } catch(CannotAcquireLockException | LockTimeoutException ex){
                        logger.info("微信APP充值回调 - 出现死锁，等待继续执行，商户订单号：{}", outTradeNo);
                        Threads.sleep(RandomUitls.randomInt(300));
                    } catch (Exception ex) {
                        logger.error("微信APP充值回调 - 处理失败，商户订单号：{}(422)", outTradeNo);
                        logger.error(ex.getMessage(), ex);
                        break;
                    }
                }
            } else {
                LogUtils.logMap("微信APP充值回调 - 验证失败，回调参数列表", params);
            }
    }
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/ips/recharge", method = RequestMethod.POST)
    public void ipsRechargeCallBack(HttpServletRequest request, HttpServletResponse response)
            throws JDOMException, IOException {
        //获取环迅支付POST过来反馈信息
        Map<String, String> params = RequestUtils.collectIPSXmlParamsToMap(request);
        logger.info("环迅支付充值回调 - 处理成功，商户订单号：{}=============", params);
        if ("000000".equalsIgnoreCase(params.get("RspCode"))
                && "Y".equalsIgnoreCase(params.get("Status"))) {
            // 对订单的处理
            String outTradeNo = params.get("IpsBillNo");// 获取支付成功的商户订单号
            ...
            while (true) {
                try {
                    ...
                    logger.info("环迅支付充值回调 - 处理成功，商户订单号：{} - end", outTradeNo);
                    break;
                } catch (ObjectOptimisticLockingFailureException | StaleObjectStateException ex) {
                    logger.info("环迅支付充值回调 - 出现并发，商户订单号：{}", outTradeNo);
                    Threads.sleep(RandomUitls.randomInt(100));
                } catch(CannotAcquireLockException | LockTimeoutException ex){
                    logger.info("环迅支付充值回调 - 出现死锁，等待继续执行，商户订单号：{}", outTradeNo);
                    Threads.sleep(RandomUitls.randomInt(300));
                } catch (Exception ex) {
                    logger.error("环迅支付充值回调 - 处理失败，商户订单号：{}(422)", outTradeNo);
                    logger.error(ex.getMessage(), ex);
                    break;
                }
            }
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/heepay/buy", method = RequestMethod.POST)
    public void heePayCallBack(HttpServletRequest request, HttpServletResponse response)
            throws JDOMException, IOException {
        heePayCallBack(request, response, false);
    }

    private void wxAppPayCallBack(HttpServletRequest request, HttpServletResponse response, boolean isH5Pay)
            throws JDOMException, IOException {
        Map<String, String> params = RequestUtils.collectXmlParamsToMap(request);
        logger.info("wxAppPayCallBack========" + params.values());
        for (String key : params.keySet()) {
            logger.info("key= "+ key + " and value= " + params.get(key));
        }
        if (params == null || params.size() == 0) {
            logger.error("无法获取微信APP支付成功回调结果");
            return;
        }
        ...
        if ("0".equalsIgnoreCase(params.get("status"))
                ...
                while (true) {
                    ...
                    try {
                        ...
                        logger.info("微信APP支付回调 - 处理成功，商户订单号：{} - end", outTradeNo);
                        break;
                    } catch (ObjectOptimisticLockingFailureException | StaleObjectStateException ex) {
                        logger.info("微信APP支付回调 - 出现并发，商户订单号：{}", outTradeNo);
                        Threads.sleep(RandomUitls.randomInt(100));
                    } catch(CannotAcquireLockException | LockTimeoutException ex){
                        logger.info("微信APP支付回调 - 出现死锁，等待继续执行，商户订单号：{}", outTradeNo);
                        Threads.sleep(RandomUitls.randomInt(300));
                    } catch (Exception ex) {
                        logger.error("微信APP支付回调 - 处理失败，商户订单：{}(411)", outTradeNo);
                        logger.error(ex.getMessage(), ex);
            ...
            LogUtils.logMap("微信APP支付回调 - 参数错误：", params);
        }
    }

    private void heePayCallBack(HttpServletRequest request, HttpServletResponse response, boolean isH5Pay)
            throws JDOMException, IOException {
        Map<String, String> params = RequestUtils.collectXmlParamsToMap(request);
        logger.info("heePayCallBack========" + params.values());
        for (String key : params.keySet()) {
            logger.info("key= "+ key + " and value= " + params.get(key));
        }
        if (params == null || params.size() == 0) {
            logger.error("无法获取汇付宝支付成功回调结果");
            return;
        }

        if ("1".equalsIgnoreCase(params.get("result"))) {
            // 对订单的处理
            String outTradeNo = params.get("agent_bill_id");// 获取支付成功的商户订单号
            String timeEndStr = params.get("time_end");
            Date payTime = DateUtilsEx.toDateFromYYYYMMDDHHMMSS(timeEndStr,
                    StringUtils.isBlank(timeEndStr) ? DateUtilsEx.getNow() : null);
            while (true) {
                ...
                try {
                    ...
                    if((boolean)resultMap.get("isSccuess")) {
                        QuartzUtils.removeJob(new QuartzPayRecord(outTradeNo, ""));
                        notifyHeePayCallbackSuccess(response);
                    ...
                    logger.info("汇付宝支付回调 - 处理成功，商户订单号：{} - end", outTradeNo);
                    break;
                } catch (ObjectOptimisticLockingFailureException | StaleObjectStateException ex) {
                    logger.info("汇付宝支付回调 - 出现并发，商户订单号：{}", outTradeNo);
                    Threads.sleep(RandomUitls.randomInt(100));
                } catch(CannotAcquireLockException | LockTimeoutException ex){
                    logger.info("汇付宝支付回调 - 出现死锁，等待继续执行，商户订单号：{}", outTradeNo);
                    Threads.sleep(RandomUitls.randomInt(300));
                } catch (Exception ex) {
                    logger.error("汇付宝支付回调 - 处理失败，商户订单：{}(411)", outTradeNo);
                    logger.error(ex.getMessage(), ex);
            ...
            LogUtils.logMap("汇付宝支付回调失败，参数列表如下：", params);
        }
    }
    ...
    public void notifyHeePayCallbackSuccess(HttpServletResponse response) throws IOException {
        response.getWriter().write("ok"); // 告诉汇付宝服务器，我收到信息了，不要在调用回调action了
    }
