...
public class GoodsTimesService {
    ...
    private static final String HQL_UNOPEN_GOODSTIME =
            "select g from GoodsTimes g where g.openTime<=?2 and g.state=?1 order by g.openTime";

    // 预设查询，不带参数条件
    private static final String HQL_QUERY_PRESET =
            "select g from GoodsTimes g where g.presetWinnerUserId is not null order by g.openTime desc";
    // 预设查询，带参数条件
    private static final String HQL_QUERY_PRESET_USERID =
            "select g from GoodsTimes g where g.presetWinnerUserId is not null and g.presetWinnerUserName=?1 order by g.openTime desc";
    ...
    public void getpreset(Page page, Map<String, String> params, boolean isExcludeOverState) {
        if (StringUtils.isEmpty(params.get("presetWinnerUserId"))
                || "".equals(params.get("presetWinnerUserId").trim())) {
            goodsTimesDao.queryPage(page, HQL_QUERY_PRESET);
        } else {
            goodsTimesDao.queryPage(page, HQL_QUERY_PRESET_USERID, params.get("presetWinnerUserId").trim());
        }
    }
    ...
    public String getVirtual(){
    	String virtual = goodsTimesDao.getIsVirtual();
        return virtual;
    }
    ...
    public List<GoodsTimes> findUnOpenGoodsTimeTop(){
        List<GoodsTimes> goodsTimesList = goodsTimesDao.findTop(
                10, HQL_UNOPEN_GOODSTIME, Const.GoodsTimesState.WAITING, new Date());
        return goodsTimesList;
    }
}
