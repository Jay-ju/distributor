 ...
 //倒计时
    function lxfEndtime(id) {
            $(".lxftime").each(function () {
                ...
                if (endtime <= nowtime&&$(this).attr("flag")==undefined) {
                    $(this).html("00:00:00"); //如果结束日期小于当前日期就提示过期啦
                    $(this).attr("flag","true");
                    change($(this).attr("goodstime"));
                } else {
                    if ($(this).attr("lxfday") == "no") {
                        ...
                        $(this).html((CHour<10?"":CHour+":") + (CMinute<10?"0"+CMinute:CMinute) + ":" + (CSecond<10?"0"+CSecond:CSecond) + ":"+(CMSecond<10?"0"+CMSecond:CMSecond)); //输出有天数的数据
                    }
                    //当前时间变化
