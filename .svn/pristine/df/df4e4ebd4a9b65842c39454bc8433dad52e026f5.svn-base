       ...
       <div class="max-box">
         ...
         <div class="record">
             ...
             <li class="plr clearfloat line">
                 <a href="${ctx}/wx/goods/goodsTimes/detail/${entity.goodsTimesId}" target="_self" class="inner">
                   <div class="r_l">
                   ...
                   <div class="r_c">
                     ...
                     <p class="large low">总需：${entity.totalTimes}</p>
                     <p class="text-heavy can large">本期参与：<em class="pink2">${entity.buyTimes}</em>人次<a href="${ctx}/wx/order/snatch/record/num/list?userId=${user.id}&goodsTimesId=${entity.goodsTimesId}" target="_self" class="blur ">查看TA的号码</a></p>
                     <p class="large low">获奖者：<a href="${ctx}/wx/order/snatch/record/others/list?userId=${entity.winngUserId}" class="blur">${entity.winngUserName}</a></p>
                     <p class="low can large">本期参与<em class="pink2">${entity.winngUserBuyTimes}</em>人次</p>
                     <p class="large low">幸运号码：<b class="pink2">${entity.luckNum}</b></p>
                     <div class="large www low">揭晓时间：<fmt:formatDate value="${entity.openTime}" pattern="yyyy-MM-dd HH:mm"/></div>
                   </div>
                 </a>
               </li>
             </c:when>
              ...
              <li class="plr clearfloat line">
                 <a href="${ctx}/wx/goods/goodsTimes/detail/${entity.goodsTimesId}" target="_self" class="inner">
                   ...
                   <div class="r_c">
                     <p class="larger text-normal www">${entity.goodsName}</p>
                     <p class="large low">参与期号：${entity.goodsTimesName}</p>
                     <span class="r_bar"><i style="width:${entity.snatchProgress}%"></i></span>
                     <p class="large low">总需：${entity.totalTimes}&nbsp;&nbsp;剩余<em class="blur">${entity.totalTimes-entity.totalBuyTimes}</em></p>
                     <p class="text-heavy can large">本期参与：<em class="pink2">${entity.buyTimes}</em>人次<a href="${ctx}/wx/order/snatch/record/num/list?userId=${user.id}&goodsTimesId=${entity.goodsTimesId}" target="_self" class="blur ">查看TA的号码</a></p>
                     <c:if test="${entity.state==2}"><div class="large www">揭晓倒计时：<b class="pink2">请稍后，正在揭晓中</b></div></c:if>
                   </div>
                 </a>
               </li>
               </c:otherwise>
...
<script type="text/javascript">
    wxcommon.autoLoad({
        ...
        url: "${ctx}/wx/order/snatch/record/others/allRecordByJson/?userId=${user.id}", // 请求url
        doRowDataFun: function(rowData){                // 格式化行级数据
            ...
            '<div class="r_c">'+
              ...
              '<p class="text-heavy can large">本期参与：<em class="pink2">'+rowData.buyTimes+'</em>人次<a href="${ctx}/wx/order/snatch/record/num/list?userId=${user.id}&goodsTimesId='+rowData.goodsTimesId+'" target="_self" class="blur ">查看TA的号码</a></p>'+
              '<p class="large low">获奖者：<a href="${ctx}/wx/order/snatch/record/others/list?userId='+rowData.winngUserId+'" class="blur">'+rowData.winngUserName+'</a></p>'+
              ...
              '<div class="large www low">揭晓时间：'+rowData.openTime.substr(0,16)+'</div>'+
            '</div>'+
          '</a>'+
            '</li>';
            }else{
